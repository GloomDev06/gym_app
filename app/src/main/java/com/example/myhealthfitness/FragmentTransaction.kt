...
class FragmentTransaction : Fragment() {
    ...
    override fun onCreateView(
        ...
        databaseReference =
            FirebaseDatabase.getInstance().reference.child("MyFitness").child("userTransactionDetails")

        val containerLayout = root.findViewById<LinearLayout>(R.id.transactionReq)
        containerLayout?.let {
                    ...
                    if (snapshot.exists()) {
                        ...
                        for (shakeSnapshot in snapshot.children) {
                            if (shakeSnapshot.hasChild("deletedAt")) {
                                ...
                                val shakeNumber =
                                    shakeSnapshot.child("productCredits").getValue(String::class.java)
                                        ?: ""
                                val timestamp =
                                    shakeSnapshot.child("currentDate").getValue(String::class.java)
                                        ?: ""

                                val action = shakeSnapshot.child("action").getValue(String::class.java)
                                ...
                                val actionTextView = shakeRequestLayout.findViewById<TextView>(R.id.action)
